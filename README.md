...
2. 浏览器访问 `192.168.4.1`，在「**WiFi 设置**」卡片中填入新名称和新密码。
...
| **中国模式 🇨🇳** | 中国、日本等「交通灯和停车标志控制」功能未上线的地区必须开启。这些地区的车机 UI 中没有该选项，导致触发条件永远不满足，设备完全无反应。开启后强制绕过此判断。 |
...
> 开启控制面板中的「**中国模式 🇨🇳**」。中国、日本等地区的车机 UI 中没有「交通灯和停车标志控制」选项，该功能被区域限制隐藏，导致设备的触发条件（`UI_fsdStopsControlEnabled` bit）永远为 0，mod 永远不触发。开启中国模式后会强制绕过此判断。
